function [Yhat,Pmod] = lrm_ab_pred(M,pt,x,Y,Seq,maxx)
%LRM_AB_PRED  Make a partial curve prediction with an LRM_AB model.
%
...
%
%   [Yhat,PostModel] = LRM_AB_PRED(M,pt,X,Y,Seq,['max'])
%    - M       : trained model
...
%
%   [[],PostModel] = LRM_AB_PRED(M,[],x,Y,Seq,['max'])
%    - M       : trained model
%    - pt      : must equal []
...
while (1)
  ...
  if (tries==MaxTries)
    fprintf(['lrm_ta_sh_pred: Integration failed, using realmin*1e100 ',...
      'instead.\n']);
    zero = find(sum(Pik,2)==0);
    Pik(zero,:) = realmin*1e100*(ones(length(zero),1)*M.Alpha');
    break;
  else
    fprintf(['lrm_ta_sh_pred: Zero membership detected, trying ', ...
        'integration again: %d\n'],tries);
    tries = tries+1;
