function [Yhat,Pmod] = lrm_cd_b_pred(M,pt,x,Y,Seq,maxx)
%LRM_CD_B_PRED  Make a partial curve prediction with an LRM_CD_B model.
%
...
%
%   [Yhat,PostModel] = LRM_CD_B_PRED(M,pt,X,Y,Seq,['max'])
%    - M       : trained model
...
%
%   [[],PostModel] = LRM_CD_B_PRED(M,[],x,Y,Seq,['max'])
%    - M       : trained model
%    - pt      : must equal []
...
%%
%%
function val = postval(pt,x,Y,Mu,order,s,u,t,sigma)
b = pt(1);
ni = length(x);
D = length(t);
val = 0;
