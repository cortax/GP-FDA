function [Yhat,Pmod] = srm_d_b_pred(M,pt,x,Y,Seq,maxx)
%SRM_D_B_PRED  Make a partial curve prediction with an SRM_D_B model.
%
...
%
%   [Yhat,PostModel] = SRM_D_B_PRED(M,pt,X,Y,Seq,['max'])
%    - M       : trained model
...
%
%   [[],PostModel] = SRM_D_B_PRED(M,[],x,Y,Seq,['max'])
%    - M       : trained model
%    - pt      : must equal []
...
%%%%%%%%%%% Estep
  ...
  for k=1:K
    ...
    for i=1:n
      ...
      ni = length(indx);
      b = fminsearch(bfun,0,SearchOps,x(indx),Y(indx,:),Mu, ...
