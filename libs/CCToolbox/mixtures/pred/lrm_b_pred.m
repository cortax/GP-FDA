function [Yhat,Pmod] = lrm_b_pred(M,pt,x,Y,Seq,maxx)
%LRM_B_PRED  Make a partial curve prediction with an LRM_B model.
%
...
%
%   [Yhat,PostModel] = LRM_B_PRED(M,pt,X,Y,Seq,['max'])
%    - M       : trained model
...
%
%   [[],PostModel] = LRM_B_PRED(M,[],x,Y,Seq,['max'])
%    - M       : trained model
%    - pt      : must equal []
  ...
  if (P>2)
    for k=1:K
      ...
      for i=1:n
        ...
        ni = length(indx);
        pt0 = [0]';  % expected value of prior on translation b
        Pmod.Eb(i,k) = ...
          fminsearch(fun,pt0,SearchOps,x(indx),Y(indx,:),Mu,P-1,sigma,s);
      end
    end
    ...
    X = [ones(N,1) x];
    for k=1:K
      Vb = 1./(sum((M.Mu(2,:,k).^2)./M.Sigma(k,:)).*lens + 1/M.S(k));
      YXMuk = Y-X*M.Mu(:,:,k);
      for i=1:n
        indx = Seq(i):Seq(i+1)-1;
        Pmod.Eb(i,k) = Vb*sum((-M.Mu(2,:,k)./M.Sigma(k,:)).*sum(YXMuk(indx,:)));
      end
    end
  end
