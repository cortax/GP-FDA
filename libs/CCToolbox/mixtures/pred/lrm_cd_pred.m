function [Yhat,Pmod] = lrm_cd_pred(M,pt,x,Y,Seq,maxx)
%LRM_CD_PRED  Make a partial curve prediction with an LRM_CD model.
%
...
%
%   [Yhat,PostModel] = LRM_CD_PRED(M,pt,X,Y,Seq,['max'])
%    - M       : trained model
...
%
%   [[],PostModel] = LRM_CD_PRED(M,[],x,Y,Seq,['max'])
%    - M       : trained model
%    - pt      : must equal []
  ...
  for k=1:K
    for d=1:D
      ...
      for i=1:n
        ...
        % values for e and f
        rXMuiS = r*(XMu'* iS);
        Pmod.Ee(i,k,d)  = (rXMuiS*Y(indx,d) +1)  ./ (rXMuiS*XMu+1);
        Pmod.Ef(i,k,d)  =  s*siR*(Y(indx,d)-XMu)   ./ (s*sum(siR)+1);
      end
    end
  end
...
if (maxx)
  ...
  for d=1:D
    YhatK = Pmod.Ee(1,:,d).*(X*Mupkd(:,:,d)) + Pmod.Ef(1,:,d);
