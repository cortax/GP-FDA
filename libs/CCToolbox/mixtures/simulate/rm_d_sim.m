function [trajs,M] = rm_d_sim(M,NumSamps,len,ops)
%RM_D_SIM  Simulate from a *RM_D model.
%   [TRAJS,M] = RM_D_SIM(M,NUM_SAMPS,[LEN],[Ops])
...
if (isempty(ops.SampleAt))
  ...
  if (any(len>lensamp))
    fprintf('RM_D_SIM: WARNING; invalid lengths, reducing max.\n');
...
for i=1:NumSamps
  ...
  for d=1:D
    ...
    trajs{i}(:,d) = X*M.Mu(:,C(i),d) + M.f(i,d) + Eps;
  end
end
M.TrueC = C;
if (ops.DoPerturb | ~isempty(ops.SampleAt))
