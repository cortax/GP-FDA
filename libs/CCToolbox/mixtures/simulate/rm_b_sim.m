function [trajs,M] = rm_b_sim(M,NumSamps,len,ops)
%RM_B_SIM  Simulate from a *RM_B model.
%   [TRAJS,M] = RM_B_SIM(M,NUM_SAMPS,[LEN],[Ops])
...
if (isempty(len))
  len = floor(rand(NumSamps,1)*(ops.SampMaxLen+1-ops.SampMinLen)) ...
    + ops.SampMinLen;
  %len = floor(randn(NumSamps,1).*1) + 3 + ops.SampMinLen;
elseif (length(len)==1)
...
if (isempty(ops.SampleAt))
  ...
  if (any(len>lensamp))
    fprintf('RM_B_SIM: WARNING; invalid lengths, reducing max.\n');
