function [Lhood,other] = lrm_d_b_like(varargin)
%LRM_D_B_LIKE  Calculate log-likelihood with LRM_D_B model.
%
%   [Lhood,Other] = LRM_D_B_LIKE(M,Trajs,[Options])
%    - M       : trained model
%    - Trajs   : 'Trajs' structure; See also CCTOOLBOX
%    - Options : see MODEL_LIKE
%
%   [Lhood,Other] = LRM_D_B_LIKE(M,X,Y,Seq,[Options])
%    - M       : trained model
...
% LRM_D_B_LIKE(M,x,Y,Seq,[Options])
else
  M = args{1};
  x = args{2};
  Y = args{3};
