function [Lhood,other] = lrm_cd_b_like(varargin)
%LRM_CD_B_LIKE  Calculate log-likelihood with LRM_CD_B model.
%
%   [Lhood,Other] = LRM_CD_B_LIKE(M,Trajs,[Options])
%    - M       : trained model
%    - Trajs   : 'Trajs' structure; See also CCTOOLBOX
%    - Options : see MODEL_LIKE
%
%   [Lhood,Other] = LRM_CD_B_LIKE(M,x,Y,Seq,[Options])
%    - M       : trained model
...
% LRM_CD_B_LIKE(M,x,Y,Seq,[Options])
else
  M = args{1};
  x = args{2};
  Y = args{3};
