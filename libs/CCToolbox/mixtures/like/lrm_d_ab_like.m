function [Lhood,other] = lrm_d_ab_like(varargin)
%LRM_D_AB_LIKE  Calculate log-likelihood with LRM_D_AB model.
%
%   [Lhood,Other] = LRM_D_AB_LIKE(M,Trajs,[Options])
%    - M       : trained model
%    - Trajs   : 'Trajs' structure; See also CCTOOLBOX
%    - Options : see MODEL_LIKE
%
%   [Lhood,Other] = LRM_D_AB_LIKE(M,x,Y,Seq,[Options])
%    - M       : trained model
...
% LRM_D_AB_LIKE(M,x,Y,Seq,[Options])
else
  M = args{1};
  x = args{2};
  Y = args{3};
