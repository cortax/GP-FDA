function [gval, basiscell, tval] = ...
                      mongrad(x, Wfdobj, basiscell, EPS, JMIN, JMAX)
...
%  WFDOBJ    ...  Functional data object defining monotone function
...
%  FVAL  ... Values of exp Wfdobj corresponding to TVAL
...
ndim  = length(coefd);
if ndim > 1 && coefd(2) ~= 1 
    error('WFDOBJ is not a single function');
end
...
if nargin == 3
    if isempty(basiscell{iter})
        bmat = eval_basis(basisobj, xiter);
        basiscell{iter} = bmat;
    else
        bmat = basiscell{iter};
    end
...
%     end
    ...
    if iter >= max([JMIN,5])
        ...
        if all(abs(dss) < EPS*max(abs(ss))) || iter == JMAX
            ...
            gval = interp1(tval, full(integfval), x, 'pchip');
            return;
        end
    end
...
warning('Wid:convergence', ...
    ['No convergence after ',num2str(JMAX),' steps in MONGRAD']);
