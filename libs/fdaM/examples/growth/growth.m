...
%  ---------------------------------------------------------------------
%  Multidimensional scaling and minimal spanning tree for temp/prec
%  ---------------------------------------------------------------------

addpath('/Users/jamesramsay/Documents/MATLAB/SpatialDA/emst')
addpath('/Users/jamesramsay/Documents/MATLAB/SpatialDA')
addpath('/Users/jamesramsay/Documents/MATLAB/MTriangle')
addpath('/Users/jamesramsay/Documents/MATLAB/fdaM')
addpath('/Users/jamesramsay/Documents/MATLAB/fdaM/examples/growth')
...
accfmat = eval_fd(agefine, accffd);
...
for i=2:54
    for j=1:i-1
        dist = 0;
        for k=1:101
            dist = dist + (accmat(k,i) - accmat(k,j))^2;
        end
        accdist(i,j) = sqrt(dist);
        accdist(j,i) = accdist(i,j);
    end
end
...
dim = 3;
[y, stress, fit] = mdscale(accdist,dim);
...
if dim == 2
    ...
    hold on
    for k=1:54
        plot(y(k,1) + agefine/10, ...
            y(k,2) + accfmat(:,k)/10, 'b-')
    end
    hold off
    axis(gca,'off')
end
...
area = sum(petstr.J);
disp(['Total area = ',num2str(area)])

Xvec  = y(:,1);
Yvec  = y(:,2);

aveintens = npts/area;
disp(['No. points per unit area = ',num2str(aveintens)])

lambda = 1e-1;
cvec0 = zeros(npts,1);

tmp = FEM_density_fit(cvec0, basisobj, Xvec, Yvec, K1, lambda);

options = optimset('Large', 'off', 'Display','iter', 'TolFun', 1e-4, ...
                             'GradObj','on', 'Hessian','off',   ...
                             'maxit', 100, 'DiffMaxChange', 1e-4);

tic;
cvec = fminunc(@FEM_density_fit, cvec0, options, basisobj, Xvec, Yvec, ...
               K1, lambda);
toc

cvec0 = cvec;

lnlamfd = fd(cvec, basisobj);

Lambda = FEM_Lambda(lnlamfd);

[F,grad] = FEM_density_fit(cvec,basisobj, Xvec, Yvec);

figure(3)
plot_FEM(lnlamfd)
axis([-15,20,-15,15,0,2])

phimat = eval_FEM_fd(Xvec, Yvec, basisobj);
lnintensity = phimat*cvec;
intensity   = exp(lnintensity);

tmat = delaunay(Xvec,Yvec);

figure(4)
trisurf(tri, Xvec, Yvec, intensity)
axis([-15,20,-15,15,0,3])

pdf = intensity/Lambda;

figure(5)
trisurf(tri, Xvec, Yvec, pdf)
axis([-15,20,-15,15,0,0.01])

%  compute minmal spanning tree

mstpath = emstFunc(y,tri);
...
if dim == 2
    for i=1:npts-1
        plot([y(mstpath(i,1),1),y(mstpath(i,2),1)], ...
             [y(mstpath(i,1),2),y(mstpath(i,2),2)], 'r-', 'linewidth', 2)
        if i == 2
            hold on
        end
    end
    hold off
    axis(gca,'off')
    title('\fontsize{16} Height Acceleration')
end

if dim == 3
    for i=1:npts-1
        plot3([y(mstpath(i,1),1),y(mstpath(i,2),1)], ...
              [y(mstpath(i,1),2),y(mstpath(i,2),2)], ...
              [y(mstpath(i,1),3),y(mstpath(i,2),3)], 'r-', 'linewidth', 2)
        if i == 2
            hold on
        end
    end
