...
elevation = [104, 154, 1117, 7, 3, 322, 668, 17, 37, 26, 17, 345, 287, ...
             30, 108, 446, 758, 27, 96, 576, 16, 550, 265, 94, 16, 261, ...
             262, 94, 302, 23, 25, 664, 235, 20, 201]';
...
plotfit_fd(tempav, daytime, daytempfd, residual, sortwrd, rng, index)
...
figure(1)
for i=1:35
    plot(daytempmat(:,i), dayprecmat(:,i), 'o-')
    xlabel('\fontsize{16} Temperature (deg C)')
    ylabel('\fontsize{16} Precipitation (mm)')
    axis([-35,20,0,15])
...
for dim=2
    ...
    if dim == 2
        ...
        hold on
        for i=1:35
            text(y(i,1)+10, y(i,2), ['\fontsize{13} ',place(i,:)])
        end
        hold off
        axis(gca,'off')
    end
    ...
    if dim == 3
        ...
        hold on
        for i=1:35
            text(y(i,1)+10, y(i,2), y(i,3), ['\fontsize{13} ',place(i,:)])
        end
        hold off
    end
    
    figure(3)
    plot(fit(:),tempprecdist(:), 'o')
...
hold on
for k=1:35
    plot(y(k,1) + daytempmat(:,k), ...
         y(k,2) + 2*dayprecmat(:,k), 'b-')
end
hold off
axis(gca,'off')
...
edges = unique([tri(:, 1:2); tri(:, 2:3); tri(:, [3 1])], 'rows');
weights = sqrt(sum((X(:, edges(:,1)) - X(:, edges(:,2))).^2));

figure(5)
update_edges(edges, X)

F=num2cell(1:length(X));
g = num2cell(ones(1, length(X)));
...
while length(F)>1
    ...
    if generation ~= g{1}
        % title('Clean-up');
        % export_pic
        valid_edges = clean_up(edges, weights, F);
    ...
    t = F{1}; gen=g{1}; F=F(2:end); g=g(2:end);
    
    d=[];
    for p=t
        hold on;
        d(end+1)=plot(X(1,p), X(2,p), 'xg',  'markersize', 12, 'linewidth', 2);
    end
    
    % export_pic
    
    [t2, F_new, best_e] = find_closest_t(edges, weights, t, F);
    F=F(F_new); F{end+1}=[t t2];
    g=g(F_new); 
    
    g{end+1}=gen+1;
    plot(X(1,edges(best_e,:)), X(2,edges(best_e,:)), 'r', 'linewidth', 2)
...
update_edges([],[])
title('\fontsize{16} Height acceleeration')
axis(gca,'off')
