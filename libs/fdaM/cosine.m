function basismat = cosine(evalarg, nbasis, period, nderiv)
%  COSINE  Computes the NDERIV derivative of the cosine series basis
...
if nderiv == 0
    %  The cosine series itself is required.
    basismat(:,1) = 1/sqrt(2);
    j    = 1:(nbasis-1);
    args = fac * j;
    basismat(:,j+1) = cos(args);
else
    %  A derivative of the cosine series is required.
    basismat(:,1) = 0.0;
