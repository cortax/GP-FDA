...
estimate_model = nhgpmodel(x_timegrid, 0.1*ones(size(x_timegrid)), log(0.1)*ones(size(x_timegrid)), log(0.1)*ones(size(x_timegrid)), log(0.1)*ones(size(x_timegrid)));
...
for j = 2:J
    ...
    display(sprintf('Iter: %i : %i',j, history(j)));
